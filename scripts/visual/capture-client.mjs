 ...
 *
 *   node scripts/visual/capture-client.mjs
 */
...
const AT = "/clients/20000000-0000-4000-8000-000000000001";

const STATES = [
  { name: "company-several-policies", url: `${HARNESS}?at=${AT}&client=full` },
  { name: "person-one-policy", url: `${HARNESS}?at=${AT}&client=single` },
  { name: "nothing-on-file", url: `${HARNESS}?at=${AT}&client=bare` },
  { name: "no-permissions", url: `${HARNESS}?at=${AT}&client=full&perms=none` },
];

const outDir = resolve(".local-visual/shots/client");
...
for (const [vp, states] of Object.entries(report.viewports)) {
  for (const [name, r] of Object.entries(states)) {
    ...
    /* The title is the client's own name. "Client Space" on screen is a failure of this capture. */
    if (/client space/i.test(r.title ?? "")) problems.push(`${vp}/${name}: the title says "Client Space"`);
  }
}
...
console.log("no empty screens, no horizontal overflow, no console errors, no 'Client Space' title");
